...
def calc_transmittances(n_fn_list, d_list, th_0, pol='s', spectral_range='narrow'):
    """
    Calculate the TRANSMISSION spectrum of a thin-film stack.

    Parameters same as calc_reflectances, but returns transmittance instead.
    """
    def extend_spectral_range(n_fn):
        """
        Starting with a narrow-spectrum refractive index function
        ...
        def extended_n_fn(lam):
            if lam < 400:
                ...
                return n_fn(lam)
        return extended_n_fn
    lam_vac_list = np.arange(360, 831)  # 360-830nm

    num_layers = len(n_fn_list)

    # Handle narrow spectral range (if needed)
    if spectral_range == 'narrow':
        n_fn_list = [extend_spectral_range(n_fn) for n_fn in n_fn_list]
    ...
    for lam_vac in lam_vac_list:
        n_list = [n_fn_list[i](lam_vac) for i in range(num_layers)]
        T = coh_tmm(pol, n_list, d_list, th_0, lam_vac)['T']  # 关键修改：提取透射率 'T'
        transmittance.append([lam_vac, T])

    return np.array(transmittance)
...
def plot_reflectances(reflectances, filename='temp_plot.png', title='Reflectance', ylabel='Fraction reflected'):
    ...
    calc_reflectances(...)
    """
    plots.spectrum_plot(reflectances, title, filename, ylabel=ylabel)
...
def plot_spectrum(spectrum, filename='temp_plot.png', title='Reflected light under illumination', ylabel='Intensity (a.u.)'):
    ...
    calc_spectrum(...)
    """
    plots.spectrum_plot(spectrum, title, filename, ylabel=ylabel)
