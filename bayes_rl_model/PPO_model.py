...
class RolloutBuffer:
    ...
    def to_tensor(self):
        self.states = torch.tensor([self.states]).to(device)
        self.actions = torch.tensor([self.actions]).to(device)
        self.logprobs = torch.tensor([self.logprobs]).to(device)
        self.state_values = torch.tensor([self.state_values]).to(device)
